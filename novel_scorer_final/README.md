...
- **一体 UI**：自包含静态 HTML 报告，双击打开即可对比排序、看单书详情、导出 CSV
...
# 采样模式（CLI 用英文 key；GUI 显示为中文 默认/三段/十段/全本）
python main.py 文件.txt --mode default   # 默认：去首尾 1000 字符后取头 5 万字（单窗口）
python main.py 文件.txt --mode three     # 三段：去首尾后均匀取 3×2 万字窗口
python main.py 文件.txt --mode ten       # 十段：去首尾后均匀取 10×2 万字窗口
python main.py 文件.txt --mode full      # 全本：整本单窗口（适合短文本）

# 参数
--merge        # 另算"合并评分"（把所有窗口拼成整体再评一次，暴露跨段复用；默认关闭）
--window N     # 覆盖窗口字数（默认 默认=5万 / 三段·十段=2万 / 全本=整本）
--nodes N      # 覆盖节点数
```

**短文兜底**：窗口互不重叠、不重复。正文不足 N×窗口时自动减少节点数到
`min(N, 正文汉字数÷窗口)`（至少 1 个窗口）；正文不足一个窗口时整段作为单窗口。
```
...
本目录已生成可执行文件 **`dist\网文评分器.exe`**（约 30MB，双击直接启动一体界面，目标机无需装 Python / jieba）。如需自行重新打包，用 PyInstaller：

```bash
python -m pip install pyinstaller
python -m PyInstaller --noconfirm --onefile --windowed --name 网文评分器 ^
  --collect-data jieba --hidden-import jieba ^
  --add-data "core;core" --add-data "ui;ui" launcher.py
```

- 入口用 `launcher.py`（双击 exe 直接进一体界面；命令行批量评分仍用 `python main.py ...`）
- `--collect-data jieba` / `--hidden-import jieba`：把 jieba 词典与模块打进包（否则运行时报找不到词典）
- `--windowed`：GUI 模式不弹控制台；`--onefile`：单文件 exe
...
├── core/
│   ├── textio.py        # 多编码读取 + 4 种采样模式（合并自 loader/sampler）
│   ├── segment.py       # 句/段切分 + RepetitionGuard 机制层
│   ├── features.py      # 多尺度特征提取
│   ├── scoring.py       # 四维评分 + 校准工具（合并自 calibrate/scoring_d1~d4）
│   └── health.py        # 环境自检 + jieba 自动安装
├── ui/
│   ├── app.py           # 一体操作界面（tkinter，零服务）
│   └── report.py        # 静态 HTML 报告生成器
└── output/              # 报告产物（report.html / results.csv / results.json）
```
